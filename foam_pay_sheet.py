...
class FoamPaySheetTopLevel(ctk.CTkToplevel):
    ...
    def add_pay_sheet_to_database(self, job_name):
        foam_pay_sheet_table = FoamPaySheetTable()
        foam_pay_sheet_table.connect()
        foam_pay_sheet_table.create_table()
        ...
        foam_pay_sheet_table.add_foam_record(job_name=job_name,
                                             closed34=values[0],
                                             closed1=values[1],
                                             closed2=values[2],
                                             closed3=values[3],
                                             ...
                                             open6=values[5],
                                             open8=values[6],
                                             bonus_amount=values[7],
                                             other_amount=values[8])

        foam_pay_sheet_table.disconnect()
    ...
    def copy_pay_sheet_template():
        ...
        template_path = os.path.join(os.getcwd(), "templates", "Foam Pay Sheet.xlsx")
        print(f"Template path: {template_path}")

        # Get the path of the destination file
        destination_path = os.path.join(os.getcwd(), "temp", "pay sheets", "foam pay sheets", "Foam Pay Sheet.xlsx")
    ...
    def create_pay_spreadsheet(job_name):
        ...
        os.rename(os.path.join(os.getcwd(), "temp", "pay sheets", "foam pay sheets", "Foam Pay Sheet.xlsx"),
                  os.path.join(os.getcwd(), "temp", "pay sheets", "foam pay sheets", file_name))

        workbook = openpyxl.load_workbook(os.path.join(os.getcwd(), "temp", "pay sheets", "foam pay sheets", file_name))
        sheet = workbook.active

        # add the job name to the spreadsheet
        sheet['A2'] = job_name

        workbook.save(os.path.join(os.getcwd(), "temp", "pay sheets", "foam pay sheets", file_name))
